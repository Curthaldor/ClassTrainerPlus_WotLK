...
ctp.Abilities = {
	...
	_storeSpellInfo = function(self, spellId, postStoreFunc)
		local spellName, spellRank, _, _, _, _, _, _, _ = GetSpellInfo(spellId)
		if not spellName then
			-- Spell not loaded yet, try again later
			-- In 3.3.5, we can't use C_Timer, so we'll use a simple frame-based delay
			...
			delayFrame:SetScript("OnUpdate", function(self, deltaTime)
				...
				if elapsed >= 0.1 then
					...
					ctp.Abilities:_storeSpellInfo(spellId, postStoreFunc)
				end
			end)
			return
		end
		
		local subText = spellRank or ""
		...
		local function store(subText)
			...
			if (ctp.SpellsToIgnoreTooltipNameFor and ctp.SpellsToIgnoreTooltipNameFor[spellId]) then
				-- Skip tooltip name processing for this spell
			end
			self._store[key] = {
				spellId = spellId
			}
		...
		if (spellsToSubstituteSubtextFor[spellId]) then
			local substituteSpellId = spellsToSubstituteSubtextFor[spellId]
			local _, substituteRank = GetSpellInfo(substituteSpellId)
			store(substituteRank or "")
		else
			store(subText)
		end
	end,
