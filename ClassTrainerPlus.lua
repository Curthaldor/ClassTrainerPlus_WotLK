...
local function SetSelected(filter)
	...
	if filter == "ignored" then 
		...
		SetTrainerServiceTypeFilter(filter, newFilterValue and 1 or 0);
	end
end

-- Hook the skill buttons for right-click (called when frame is shown)
local buttonsHooked = false
local function HookSkillButtons()
	if buttonsHooked then return end
	
	for i = 1, 11 do
		local button = _G["ClassTrainerPlusSkill" .. i]
		if button then
			-- Make sure buttons can receive right clicks
			button:RegisterForClicks("LeftButtonUp", "RightButtonUp")
			-- Override the OnClick handler
			button:SetScript("OnClick", ClassTrainerPlusSkillButton_OnClick)
		end
	end
	buttonsHooked = true
end
...
function ClassTrainerPlusFrame_Show()
	-- Frame is already showing when this is called from OnShow
	-- Don't call ShowUIPanel here as it causes recursion
	
	-- Hook the buttons to enable right-click
	HookSkillButtons()
	
	-- Make sure ignore store is loaded before updating
	...
	SetTrainerServiceTypeFilter("available", TRAINER_FILTER_AVAILABLE and 1 or 0)
	SetTrainerServiceTypeFilter("unavailable", TRAINER_FILTER_UNAVAILABLE and 1 or 0)
	SetTrainerServiceTypeFilter("used", TRAINER_FILTER_USED and 1 or 0)
	
	-- Expand all skill line headers by default
...
function ClassTrainerPlusFrame_OnLoad(self)
	...
	-- Initialize the filter dropdown menu
	UIDropDownMenu_Initialize(ClassTrainerPlusFilterDropdown, function(self, level)
		local info = UIDropDownMenu_CreateInfo()
		
		info.text = AVAILABLE
		info.func = function() SetSelected("available") end
		info.checked = IsSelected("available")
		info.keepShownOnClick = true
		info.colorCode = "|cff00ff00" -- Green for available
		UIDropDownMenu_AddButton(info)
		
		info.text = UNAVAILABLE
		info.func = function() SetSelected("unavailable") end
		info.checked = IsSelected("unavailable")
		info.keepShownOnClick = true
		info.colorCode = "|cffff1a1a" -- Red for unavailable
		UIDropDownMenu_AddButton(info)
		
		info.text = USED
		info.func = function() SetSelected("used") end
		info.checked = IsSelected("used")
		info.keepShownOnClick = true
		info.colorCode = "|cff808080" -- Gray for used/already known
		UIDropDownMenu_AddButton(info)
		
		info.text = ctp.L.IGNORED
		info.func = function() SetSelected("ignored") end
		info.checked = IsSelected("ignored")
		info.keepShownOnClick = true
		info.colorCode = "|cffffd100" -- Yellow for ignored
		UIDropDownMenu_AddButton(info)
	end)
	UIDropDownMenu_SetSelectedValue(ClassTrainerPlusFilterDropdown, AVAILABLE)
	UIDropDownMenu_SetText(ClassTrainerPlusFilterDropdown, FILTER)
	UIDropDownMenu_SetWidth(ClassTrainerPlusFilterDropdown, 105)
end

function ClassTrainerPlus_OnSearchTextChanged(self)
	-- SearchBoxTemplate_OnTextChanged doesn't exist in 3.3.5
	-- The search functionality still works without it
...
function ClassTrainerPlusFrame_Update()
	...
	for i = 1, CLASS_TRAINER_SKILLS_DISPLAYED, 1 do
		...
		if (skillIndex <= numFilteredTrainerServices) then
			...
			if (serviceType == "header") then
				...
				-- Prevent the texture from stretching - must be done after SetNormalTexture
				local normalTexture = skillButton:GetNormalTexture()
				if normalTexture then
					normalTexture:ClearAllPoints()
					normalTexture:SetWidth(16)
					normalTexture:SetHeight(16)
					normalTexture:SetPoint("LEFT", skillButton, "LEFT", 0, 0)
				end
				_G["ClassTrainerPlusSkill" .. i .. "Highlight"]:SetTexture("Interface\\Buttons\\UI-PlusButton-Hilight")
			else
				skillButton:SetNormalTexture(nil)
				...
				if (serviceSubText and serviceSubText ~= "") then
					...
					skillText:SetWidth(0)
					-- Right-align the subtext like the default trainer
					skillSubText:ClearAllPoints()
					skillSubText:SetPoint("RIGHT", skillButton, "RIGHT", -3, 0)
					skillSubText:SetJustifyH("RIGHT")
					skillSubText:Show()
				else
					skillSubText:Hide()
				...
				local subTextR, subTextG, subTextB
				if (serviceType == "available") then
					if (not service.isIgnored) then
						skillButton:SetNormalFontObject("GameFontNormalLeftGreen")
						-- Match subtext color to main text (green for available)
						subTextR, subTextG, subTextB = 0, 1, 0
					else
						skillButton:SetNormalFontObject("ClassTrainerPlusIgnoredFont")
						-- Match subtext color for ignored (yellowish)
						subTextR, subTextG, subTextB = 1, 0.82, 0
					end
				elseif (serviceType == "used") then
					skillButton:SetNormalFontObject("GameFontDisable")
					-- Match subtext color to disabled (gray)
					subTextR, subTextG, subTextB = 0.5, 0.5, 0.5
				else
					...
					-- Match subtext color to main text (red for unavailable)
					subTextR, subTextG, subTextB = 1, 0.1, 0.1
				end
				-- Store the color so it can be restored on mouse leave
				skillSubText.normalR = subTextR
				skillSubText.normalG = subTextG
				skillSubText.normalB = subTextB
				skillSubText:SetTextColor(subTextR, subTextG, subTextB)
			end
			...
			end  -- Close the else block from line 389
		else
			skillButton:Hide()
		end
	end
	...
	-- Prevent the texture from stretching - must be done after SetNormalTexture
	local allButtonTexture = ClassTrainerPlusCollapseAllButton:GetNormalTexture()
	if allButtonTexture then
		allButtonTexture:ClearAllPoints()
		allButtonTexture:SetWidth(16)
		allButtonTexture:SetHeight(16)
		allButtonTexture:SetPoint("LEFT", ClassTrainerPlusCollapseAllButton, "LEFT", 0, 0)
	end
end
...
function ClassTrainerPlus_SetSelection(id)
	...
	if (reqLevel > 1) then
		separator = ", "
		-- In 3.3.5, pet spells are trained at separate pet trainers
	...
	if (isProfession) then
		...
		-- In 3.3.5, the game handles profession limit checks internally
		-- so we don't need to check GetNumPrimaryProfessions here
	else
...
UIDropDownMenu_Initialize(menuFrame, function(self, level)
	-- Don't try to get service if serviceId isn't set yet
	if not self.serviceId then
		return
	end
	
	local service = ctp.TrainerServices:GetService(self.serviceId)
	if (not service or service.type == "header" or service.type == "used") then
		return
	end
	
	local info = UIDropDownMenu_CreateInfo()
	info.isTitle = true
	info.text = service.menuTitle
	...
	info.text = ctp.L["IGNORED"]
	info.checked = service.isIgnored
	...
	if (ctp.SpellsToIgnoreAllRanksOn == nil or not ctp.SpellsToIgnoreAllRanksOn[spellId]) then
		...
		if (spellIds ~= nil) then
			...
			info = UIDropDownMenu_CreateInfo()
			info.text = ctp.L["IGNOREALLRANKS"]
			info.checked = allIgnored
			info.keepShownOnClick = false
			info.func = function()
				PlaySound("UChatScrollButton")
...
function ClassTrainerPlusSkillButton_OnClick(self, button)
	...
	elseif (button == "RightButton") then
		...
		CloseDropDownMenus()
		-- Set the service ID for the menu
		ClassTrainerPlusToggleFrame.serviceId = self:GetID()
		-- Show the dropdown menu
		ToggleDropDownMenu(1, nil, ClassTrainerPlusToggleFrame, self, 0, 0)
	end
end
